...
import { 
    Mic2, Users, Disc, PartyPopper, Star, Guitar, Wand2, Music, 
    Lock, Cpu, HeartHandshake, Trophy, Ticket, ArrowLeft, ArrowRight,
...
const ArtistAnt = () => {
    ...
    const handleArtistSubmit = async (e) => {
        ...
        try {
            ...
            addToast("Something went wrong while submitting. Please try again.", 'error');
        } finally {
            setIsSubmitting(false);
        }
    };
    ...
    const handleUpdateProfile = async (e) => {
        ...
        setIsSubmitting(true);
        try {
            await updateArtist(artistProfile.id, artistProfile);
            setSuccessMessage({
    ...
    const handleClientSubmit = async (e) => {
        e.preventDefault();
        // For client requests, we don't strictly require login if we want a guest experience, 
        // but let's stick to the current logic for consistency.
        if (!user) { setAuthModal(true); return; }
        setIsSubmitting(true);
        try {
    ...
    const handleDeleteProfile = async () => {
        ...
        try {
            ...
            addToast("Couldn't delete your profile. Please try again.", 'error');
        } finally {
            setIsSubmitting(false);
        }
    };
    ...
    if (!authInitialized) {
        return <div className="min-h-screen bg-black flex items-center justify-center"><Loader2 className="animate-spin text-[#FF6B6B]" size={32} /></div>;
    }
        ...
        <div className="min-h-screen bg-[#050505] text-white relative font-outfit scroll-smooth">
            ...
            <div className="fixed top-4 left-4 right-4 md:top-8 md:left-8 md:right-8 z-[100] flex justify-between items-center">
                {isFormView ? (
                    ...
                    <a href="https://newbi.live" target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 md:gap-4 px-4 py-3 md:px-6 md:py-4 bg-white/5 hover:bg-white/10 border border-white/10 rounded-2xl md:rounded-[1.5rem] backdrop-blur-3xl transition-all group">
                        <Globe size={16} className="text-[#FF6B6B] group-hover:scale-110 transition-transform" />
                ...
                {successMessage && (
                    ...
                    >
                        ...
                        >
                            ...
                            <div className="w-24 h-24 rounded-full bg-green-500/20 border border-green-500/20 flex items-center justify-center mx-auto relative z-10">
                            ...
                            <button 
                                ...
                                className="w-full h-20 bg-white text-black rounded-[1.5rem] font-black uppercase tracking-[0.2em] text-[11px] hover:bg-[#7B61FF] hover:text-white transition-all shadow-xl active:scale-95 relative z-10"
                            >
                                ACKNOWLEDGE
                            </button>
                        </motion.div>
            ...
            <AnimatePresence mode="wait">
                            ...
                            <div className="max-w-7xl mx-auto">
                                <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-12 md:mb-16">
                                    ...
                                    <div className="flex items-center gap-3">
                                        <button 
                                            onClick={() => document.getElementById('talent-carousel')?.scrollBy({ left: -340, behavior: 'smooth' })}
                                            className="w-12 h-12 md:w-14 md:h-14 rounded-2xl bg-zinc-900 border border-white/5 flex items-center justify-center text-white/40 hover:text-white transition-all group"
                                        >
                                ...
                                <div id="talent-carousel" className="flex overflow-x-auto pb-8 gap-6 md:gap-8 scrollbar-hide snap-x snap-mandatory scroll-smooth">
                        ...
                        <section className="relative py-16 md:py-24 px-6 md:px-8 bg-[#020202] border-t border-white/5">
                            <div className="max-w-7xl mx-auto text-center space-y-12 md:space-y-16">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-10 text-left">
                                    <button 
                                        onClick={() => setView('artist_form')}
                                        className="p-8 md:p-12 bg-zinc-900/20 border border-white/10 rounded-[3rem] space-y-8 group hover:border-[#FF6B6B]/30 transition-all hover:bg-zinc-900/40"
                                    >
                                        ...
                                        <div className="space-y-4">
                                            <div className="flex items-center justify-between">
                                                ...
                                                <ArrowRight size={24} className="text-gray-700 group-hover:text-[#FF6B6B] group-hover:translate-x-2 transition-all" />
                                            </div>
                                    ...
                                    <button 
                                        onClick={() => setView('client_form')}
                                        className="p-8 md:p-12 bg-zinc-900/20 border border-white/10 rounded-[3rem] space-y-8 group hover:border-[#7B61FF]/30 transition-all hover:bg-zinc-900/40"
                                    >
                                        ...
                                        <div className="space-y-4">
                                            <div className="flex items-center justify-between">
                                                ...
                                                <ArrowRight size={24} className="text-gray-700 group-hover:text-[#7B61FF] group-hover:translate-x-2 transition-all" />
                                            </div>
                                            <p className="text-gray-500 font-medium text-[14px] leading-relaxed">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                                    {ECOSYSTEM_FEATURES.map((feature, i) => (
                                        ...
                                        >
                                            <div className="space-y-6">
                                                <div className="flex items-center justify-between">
                                                    <div className="w-14 h-14 bg-white/5 rounded-2xl flex items-center justify-center text-[#7B61FF] group-hover:scale-110 transition-all">
                                ...
                                <form onSubmit={
                                    ...
                                    <AnimatePresence mode="wait">
                                        {view === 'artist_form' ? (
                                            <motion.div key={`artist-step-${step}`} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-12">
                                                {step === 1 && (
                                                    <div className="space-y-12">
                                                        <div className="flex flex-col items-center gap-8 bg-white/[0.02] border border-white/5 rounded-[3rem] p-10 md:p-12">
                                                        ...
                                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-16 gap-y-12">
                                                            <FormField label="Professional Name" value={artistData.name} onChange={(e) => setArtistData({...artistData, name: e.target.value})} placeholder="e.g. DJ Shadow" required />
                                                            <FormField label="Direct Phone" value={artistData.phone} onChange={(e) => setArtistData({...artistData, phone: e.target.value})} placeholder="+91 XXXX XXXX" required />
                                                            <FormField label="Years of Experience" type="number" value={artistData.experienceYears} onChange={(e) => setArtistData({...artistData, experienceYears: e.target.value})} placeholder="e.g. 5" required />
                                                            <FormSelect label="Current Base" value={artistData.city} onChange={(e) => setArtistData({...artistData, city: e.target.value})} options={PREDEFINED_CITIES} required />
                                                            <div className="md:col-span-2">
                                                                <FormSelect label="Performance Vertical" value={artistData.categories} onChange={(e) => setArtistData({...artistData, categories: e.target.value})} options={TALENT_CATEGORIES.map(c => c.label)} required />
                                                            </div>
                                                        </div>
                                                    </div>
                                                )}
                                                {step === 2 && (
                                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-16 gap-y-12">
                                                        <FormField label="Instagram Handle" value={artistData.instagram} onChange={(e) => setArtistData({...artistData, instagram: e.target.value})} placeholder="@handle" required />
                                                        ...
                                                        <div className="md:col-span-2">
                                                            <FormField label="Primary Portfolio Link" value={artistData.portfolio} onChange={(e) => setArtistData({...artistData, portfolio: e.target.value})} placeholder="YouTube, Soundcloud, or Website" required />
                                                        </div>
                                                    </div>
                                                )}
                                                {step === 3 && (
                                                    <div className="space-y-8">
                                                        <FormField label="Professional Bio" isTextArea value={artistData.bio} onChange={(e) => setArtistData({...artistData, bio: e.target.value})} placeholder="Describe your style, experience, and the vibe you bring to a show..." required />
                                                    </div>
                                                )}
                                            </motion.div>
                                        ) : (
                                            <motion.div key={`client-step-${step}`} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="space-y-12">
                                                {step === 1 && (
                                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-16 gap-y-12">
                                                        <FormField label="Authorized Representative" value={clientData.name} onChange={(e) => setClientData({...clientData, name: e.target.value})} placeholder="Full name" required />
                                                        <FormField label="Organization" value={clientData.org} onChange={(e) => setClientData({...clientData, org: e.target.value})} placeholder="Company or Entity" required />
                                                        <FormField label="Business Email" value={clientData.email} onChange={(e) => setClientData({...clientData, email: e.target.value})} placeholder="email@company.com" required />
                                                        <FormSelect label="Event Location" value={clientData.city} onChange={(e) => setClientData({...clientData, city: e.target.value})} options={PREDEFINED_CITIES} required />
                                                    </div>
                                                )}
                                                {step === 2 && (
                                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-16 gap-y-12">
                                                        <FormField label="Required Talent Type" value={clientData.category} onChange={(e) => setClientData({...clientData, category: e.target.value})} placeholder="Genre or category" required />
                                                        <FormField label="Allocated Budget" value={clientData.budget} onChange={(e) => setClientData({...clientData, budget: e.target.value})} placeholder="Range" required />
                                                        <div className="md:col-span-2">
                                                            <FormField label="Gig Requirements" isTextArea value={clientData.requirement} onChange={(e) => setClientData({...clientData, requirement: e.target.value})} placeholder="Date range, venue details, and specific requirements..." required />
                                                        </div>
                                                    </div>
                                                )}
                                            </motion.div>
                                    ...
                                    <div className="pt-12 flex items-center gap-10">
                                        <button 
                                            ...
                                            className="w-full max-w-5xl h-20 md:h-24 px-12 md:px-20 bg-white text-black font-black uppercase tracking-[0.3em] text-[11px] rounded-2xl hover:scale-[1.01] active:scale-95 transition-all flex items-center justify-center gap-4 shadow-[0_20px_60px_rgba(255,255,255,0.1)]"
                                        >
                ...
                {view === 'dashboard' && artistProfile && (
                    <motion.div key="dashboard" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-7xl mx-auto pt-32 md:pt-56 px-6 md:px-8 pb-40">
                        <div className="bg-zinc-900/40 backdrop-blur-3xl border border-white/10 rounded-[3.5rem] p-10 md:p-16 flex flex-col md:flex-row justify-between items-center gap-12 relative overflow-hidden">
                            ...
                            <div className="space-y-6 text-center md:text-left relative z-10">
                                ...
                                <div className="flex items-center gap-6 justify-center md:justify-start">
                                    ...
                                    <div className="text-left">
                                        ...
                                        <p className="text-lg font-black text-white uppercase italic tracking-widest">
                                            {artistProfile.profileStatus === 'pending' ? 'PENDING CERTIFICATION' : 'VERIFIED ARTIST'}
                                        </p>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 mt-12">
                            <div className="lg:col-span-2 space-y-8">
                                ...
                                <div className="bg-zinc-900/20 border border-white/5 rounded-[3rem] p-16 md:p-24 text-center flex flex-col items-center gap-8 group">
                            ...
                            <div className="space-y-8">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <button 
                                        ...
                                        className="w-full flex items-center justify-between p-8 bg-white/5 hover:bg-white/10 border border-white/5 rounded-2xl transition-all group"
                                    >
                                        <div className="flex items-center gap-5">
                                            <div className="w-10 h-10 bg-white/5 rounded-xl flex items-center justify-center">
                                                <ArrowLeft size={20} className="text-gray-500" />
                                            </div>
                                            <span className="text-[11px] font-black uppercase tracking-widest text-gray-500 group-hover:text-white transition-colors">Sign Out</span>
                                        </div>
                                        <ChevronRight size={18} className="text-gray-500/40 group-hover:translate-x-1 transition-transform" />
                                    </button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="bg-zinc-900/60 backdrop-blur-3xl border border-white/10 rounded-[3.5rem] p-10 md:p-16 shadow-2xl space-y-12">
                            ...
                            <div className="pt-10 border-t border-white/5 flex justify-end">
                                <button 
                                    ...
                                    disabled={isSubmitting}
                                    className="h-24 px-16 bg-gradient-to-r from-[#FF6B6B] to-[#7B61FF] text-white font-black uppercase tracking-[0.3em] text-[12px] rounded-2xl hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-4 shadow-[0_20px_50px_rgba(255,107,107,0.3)]"
                                >
            ...
            <AnimatePresence>
                {showDeleteConfirm && (
                    ...
                    >
                        <motion.div 
                            initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }}
                            className="w-full max-w-md bg-zinc-900 border border-white/10 rounded-[3rem] p-12 text-center space-y-10 shadow-2xl"
                        >
                            ...
                            <div className="flex flex-col gap-6">
                                <button 
                                    ...
                                    disabled={isSubmitting}
                                    className="h-20 w-full bg-red-500 text-white font-black uppercase tracking-widest text-[11px] rounded-xl hover:bg-red-600 transition-all flex items-center justify-center gap-3"
                                >
                                    {isSubmitting ? <Loader2 className="animate-spin" /> : 'YES, DELETE PROFILE'}
                                </button>
                                <button 
                                    onClick={() => setShowDeleteConfirm(false)}
                                    className="h-20 w-full bg-white/5 text-gray-500 font-black uppercase tracking-widest text-[11px] rounded-xl hover:bg-white/10 transition-all"
                                >
                                    Cancel
                                </button>
                            </div>
